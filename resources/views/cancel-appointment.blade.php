    ...
    <div class="location-section max-w-7xl mx-auto py-12 px-6 sm:px-8">
        ...
        <div class="relative z-10 max-w-md mx-auto px-4 sm:px-0 my-8">
            <div class="bg-white bg-opacity-90 p-6 rounded-lg shadow-lg">
                ...
                <form action="{{ route('appointments.cancel.confirm', $appointment->token) }}" method="POST" class="space-y-4">
                    @csrf
                    <h2 class="text-2xl font-bold text-gray-800 mb-6 text-center">Cancelar Cita:</h2>

                    <!-- Información de la cita -->
                    <div class="space-y-4">
                        <p><strong>Paciente:</strong> {{ $appointment->patient->name }} {{ $appointment->patient->surnames }}</p>
                        <p><strong>Fecha:</strong> {{ date('d/m/Y', strtotime($appointment->timeSlot->availableHour->availableDate->date)) }}</p>
                        <p><strong>Hora:</strong> {{ date('H:i', strtotime($appointment->timeSlot->start_time)) }}</p>
